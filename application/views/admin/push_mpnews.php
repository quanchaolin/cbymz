        ...
        <div class="box">
            <div class="nav-tabs-custom" id="tabbable">
                <ul class="nav nav-tabs">
                    ...
                    <li class="active"><a href="javascript:void(0);">图文消息（点击跳转到图文消息页面）</a></li>
...
<script id="contentListTemplate" type="x-tmpl-mustache">
    {{#contentList}}
        <tr role="row" class="odd" data-id="{{id}}">
            <td><input type="radio" name="image_item" value="{{media_id}}" data-name={{name}}></td>
            <td><img src="{{thumb_url}}" width="80" height="80" /></td>
            <td>{{#showContent}}{{/showContent}}</td>
...
<script type="application/javascript">
    $(function () {
        ...
        function loadContentList() {
            ...
            var url = "<?=site_url('admin/material_news/news_lists/')?>" ;
            $.ajax({
                type:'get',
                dataType:"json",
                url : url,
                ...
                if (result.data.total > 0){
                    ...
                    var rendered = Mustache.render(contentListTemplate, {
                        ...
                        showContent:function(){
                            var news_item=this.news_item
                                ,news_html='<table>';
                            return function(){
                                for(var i in news_item){
                                    news_html+='<tr><td>'+news_item[i]['title']+'<td><tr>';
                                }
                                news_html+='</table>';
                                return news_html;
                            }
