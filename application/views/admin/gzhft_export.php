...
// 设置属性
$objPHPExcel->getProperties()->setCreator('广种福田心愿单');
$objPHPExcel->getProperties()->setLastModifiedBy('广种福田心愿单');
...
$objPHPExcel->getActiveSheet()->SetCellValue('G1','总金额');
...
foreach($list as $key=>$value):
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('G'.($key+2),$value['order_total_price']);
    $objPHPExcel->getActiveSheet()->SetCellValue('H'.($key+2),$value['add_time']);


endforeach;

// Rename sheet
$objPHPExcel->getActiveSheet()->setTitle('广种福田心愿单');
...
header("Content-Disposition:attachment;filename=广种福田心愿单$date.xls");
