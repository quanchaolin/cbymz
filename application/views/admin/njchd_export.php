...
// 设置属性
$objPHPExcel->getProperties()->setCreator('念经超度心愿单');
$objPHPExcel->getProperties()->setLastModifiedBy('念经超度心愿单');
...
$objPHPExcel->getActiveSheet()->SetCellValue('E1','邮箱地址');
$objPHPExcel->getActiveSheet()->SetCellValue('F1','功德主所求愿望');
$objPHPExcel->getActiveSheet()->SetCellValue('G1','超度往生者姓名');
$objPHPExcel->getActiveSheet()->SetCellValue('H1','总金额');
...
foreach($list as $key=>$value):
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('E'.($key+2),$value['receiver_email']);
    $objPHPExcel->getActiveSheet()->SetCellValue('F'.($key+2),$value['content']);
    $objPHPExcel->getActiveSheet()->SetCellValue('G'.($key+2),$value['other_name']);
    $objPHPExcel->getActiveSheet()->SetCellValue('H'.($key+2),$value['order_total_price']);
    $objPHPExcel->getActiveSheet()->SetCellValue('I'.($key+2),$value['add_time']);

endforeach;

// Rename sheet
$objPHPExcel->getActiveSheet()->setTitle('念经超度心愿单');
...
header("Content-Disposition:attachment;filename=念经超度心愿单$date.xls");
