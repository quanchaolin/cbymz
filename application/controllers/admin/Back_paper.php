...
class Back_paper extends Content
{
    ...
    {
        $this->name = '回向文';
        $this->control = 'back_paper';
        $this->list_view = 'back_paper_list'; // 列表页
        $this->add_view = 'back_paper_add'; // 添加页
        parent::__construct();
        $this->baseurl = site_url('admin/back_paper/');
        $this->load->model('admin/back_paper_model', 'model');
    ...
    public function add()
    {
        ...
        $this->load->view('admin/back_paper_'.$msgtype);
    }
    ...
    public function edit()
    {
        ...
        $value['back_paper_item']=array();
        if($value['msgtype']=='news')
        {
            $value['back_paper_item']=$back_paper_item=$this->back_paper_item_model->lists('*',array('back_paper_id'=>$value['id']),'id ASC');
        }
        $data['value'] = $value;
        $msgtype=$value['msgtype'];
        $this->load->view('admin/back_paper_'.$msgtype, $data);
    }
        ...
        if ($id) { // 修改 ===========
            ...
            foreach($list as $item)
            {
                ...
                if($item['back_paper_item_id']!=''){
                    $this->back_paper_item_model->update($insert_data,array('id'=>$item['back_paper_item_id']));
                }else{
                    $item_insert_id=$this->back_paper_item_model->insert($insert_data);
                    $url=$item['item_url'];
                    if($url=='')
                    {
                        $url=site_url('back_paper/show?id='.$item_insert_id);
                    }
                    $this->back_paper_item_model->update(array('url'=>$url),array('id'=>$item_insert_id));
                }
            }
            response_msg(0,'修改成功!');
        } else { // ===========添加 ===========
    ...
    public function preview()
    {
        ...
        foreach($list as $val){
            ...
            {
                case 'text':
                    $result=$wechat->sendText($val,$row['content']);
                    break;
                case 'image':
                    $result=$wechat->sendImage($val,$row['media_id']);
                    break;
                case 'voice':
                    $result=$wechat->sendVoice($val,$row['media_id']);
                    break;
                case 'video':
                    $result=$wechat->sendVideo($val,$row['media_id'],$row['title'],$row['description']);
                    break;
                case 'music':
                    $result=$wechat->sendMusic($val,$row['title'],$row['description'],$row['musicurl'],$row['hqmusicurl'],$row['thumb_media_id']);
                    break;
                case 'news':
                    $back_paper_item=$this->back_paper_item_model->lists('*',array('back_paper_id'=>$row['id']),'id ASC');
                    $news_data=array();
                    ...
                    if($news_data){
                        $result=$wechat->messageCustomSend($val,$news_data,'news');
                    }
                    break;
                case 'mpnews':
                    $result=$wechat->sendNewsOnce($val,$row['media_id']);
                    break;
            }
        }
