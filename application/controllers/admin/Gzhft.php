...
class Gzhft extends Content
{
    ...
    function __construct()
    {
        $this->name = '广种福田';
        $this->control = 'gzhft';
        $this->list_view = 'gzhft_list'; // 列表页
        $this->add_view = 'gzhft_add'; // 添加页
        parent::__construct();
        $this->baseurl = site_url('admin/gzhft/');
    ...
    public function index(){
        $product_group=$this->product_group_model->row(array('id'=>2,'status'=>1));
        if(!$product_group)
        {
            echo json_encode(
                array(
    ...
    public function export()
    {
        ...
        $data['list']=$list;
        $this->load->view('admin/gzhft_export',$data);
    }
}
