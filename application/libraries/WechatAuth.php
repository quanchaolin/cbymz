...
class WechatAuth {
    ...
    public function getAccessToken($type = 'client', $code = null){
        if($type == 'client')
        {
            $access_token=json_decode(file_get_contents(APPPATH.'cache/access_token.json'));
        ...
        if(is_array($token)){
            if(isset($token['errcode'])){
                ...
                //保存公众号的access_token到数据库
                if($type == 'client')
                {
                    $new_access_token['access_token']=$token['access_token'];
                    $new_access_token['expires_in']=$token['expires_in'];
                    $new_access_token['expires']=time()+7000;
                    $fp=fopen(APPPATH.'cache/access_token.json','w');
                    fwrite($fp,json_encode($new_access_token));
                    fclose($fp);
                    $access_token=json_decode(file_get_contents(APPPATH.'cache/access_token.json'));
                    return $access_token;
                }else{
    ...
    public function messageCustomSend($openid, $content, $type = self::MSG_TYPE_TEXT){
        ...
        //根据类型附加额外数据
        $data[$type] = call_user_func(array('WechatAuth', $type), $content);
     ...
     * @param  string $media  音频ID
     */
    public function sendMpnews($openid, $media){
        return $this->messageCustomSend($openid, $media, self::MSG_TYPE_MPNEWS);
    }
    /**
     * 创建用户组
     ...
     */
    public function qrcodeCreate($scene_id_str='',$action_name=self::QR_SCENE, $expire_seconds = 0){
        $data = array();
        $scene=array();
        $data['action_name']=$action_name;
        if($action_name==self::QR_SCENE || $action_name==self::QR_LIMIT_SCENE)
        {
            $scene['scene_id']=$scene_id_str;
        ...
        {
            $scene['scene_str']=$scene_id_str;
            if($action_name==self::QR_STR_SCENE)
            {
                $data['expire_seconds']=$expire_seconds;
            }
        }
     ...
     * @param  integer $media 语音ID
     */
    private static function mpnews($media){
        $data['media_id'] = $media;
        return $data;
    }
}