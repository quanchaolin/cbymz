<?php
/*
	[BBRMEDIA] (C) 2010 bbrmedia Inc.
	$Id: jaqy 2010-10-15 10:15:00   
*/

/*
    usage:
    自动裁切:
    程序会按照图片的尺寸从中部裁切最大的正方形或裁剪位置起点坐标从0开始，并按目标尺寸进行缩略
 
    $t->setSrcImg("images/test.jpg");
    $t->setCutType(1);      //这一句就OK了
	$t->imgCutSign(0);  //为0裁剪位置起点坐标从0开始, 为1程序会按照图片的尺寸从中部裁切最大的正方形，并按目标尺寸进行缩略 ,默认为0
...
*/
 ...
 /*
    usage:
	$gdclass = new My_image_class;	
	$gdclass->setSrcImg("001.jpg"); //图片源地址
	//$gdclass->setCutType(1); //以高度为参自动剪图
	$gdclass->mask_word = "wojia99.com";//加水印文字
	$gdclass->mask_font_color = "#ffffff";  //水印文字的颜色
	$gdclass->font = 1;///水印文字
	$gdclass->font_size = 12;//水印文字的大小
	//$gdclass->mask_img = "maskimg.jpg"; //加水印图片
	$gdclass->setDstImg("100.jpg");  //输出图片地址,去掉则显示图片
	$gdclass->createImg(200,250);    //输出图片宽,高
	*/	
$FTIMG="jpg,gif,png,wbmp,jpeg";           //当前GD库支持的类型 
...
class My_image_class
{
    ...
    var $h_dst;// 新图句柄
    var $h_mask;// 水印句柄
    var $img_create_quality = 90;// 图片生成质量
    var $img_display_quality = 90;// 图片显示质量,默认为75
    var $img_scale = 10;// 图片缩放比例
    var $src_w = 0;// 原图宽度
    var $src_h = 0;// 原图高度
    var $dst_w = 0;// 新图总宽度
    var $dst_h = 0;// 新图总高度
    ...
    var $mask_offset_x = 38;// 水印横向偏移
    var $mask_offset_y = 38;// 水印纵向偏移
    var $font_w=0;// 水印字体宽
    var $font_h=0;// 水印字体高
    var $mask_w;// 水印宽
    var $mask_h;// 水印高
    ...
    var $font_size;// 尺寸
    var $mask_position = 1;// 水印位置
    var $mask_img_pct = 100;// 图片合并程度,值越大，合并程序越低
    var $mask_txt_pct = 100;// 文字合并程度,值越小，合并程序越低
    var $img_border_size = 0;// 图片边框尺寸
    var $img_border_color="#FFFFFF";// 图片边框颜色
    var $_flip_x=0;// 水平翻转次数
    var $_flip_y=0;// 垂直翻转次数
    
    var $cut_type=0;// 剪切类型
    var $cut_align=0; //为0裁剪位置起点坐标从0开始, 为1程序会按照图片的尺寸从中部裁切最大的正方形，并按目标尺寸进行缩略
    var $outformat;  //输出格式
    var $img_type;// 文件类型
    ...
    */
      //取得图片的宽
     function imgCutSign($n) {
	   $this->cut_align = (int)$n;
	 }
    ...
    function _createMask()
    {
        ...
        if($this->mask_img)
        {
            ...
            else
            {
                // 创建新图并拷贝
                $this->h_dst = imagecreatetruecolor($this->dst_w, $this->dst_h);
                $white = ImageColorAllocate($this->h_dst,10,10,10);
                imagefilledrectangle($this->h_dst,0,0,$this->dst_w,$this->dst_h,$white);// 填充背景色
				//imagecolortransparent($this->h_dst,$white); //使背景透明 add --
                $this->_drawBorder();
                imagecopyresampled( $this->h_dst, $this->h_src,
                                    $this->start_x, $this->start_y,
                                    $this->src_x, $this->src_y,
    ...
    function _output()
    {
		...
		if(!empty($this->outformat)) $img_type = $this->outformat;   //---------------
                ...
                case 1:
                    ...
                    if($rate_w >= 1 && $rate_h >=1){
                        if($this->src_w > $this->src_h){
                            ...
                            $src_y = $this->cut_align ? round($this->src_h-$this->src_w)/2 : 0;
	...
	//rotatefunc
function imageRotateOwn($src_img, $angle, $bicubic=false) {
...
function outTurnByImg($intoimg1,$intoimg2,$intoimg3,$resultimg,$bgpng){
	   //header('Content-type: image/png');
	   $temByBgpng = $bgpng.'temp.png';
	   $outimg2 = $intoimg2.".turn2.png";
	   $outimg3 = $intoimg3.".turn3.png";
	   $temimgs = $intoimg3.".temp.png";
	   $temjpgs = $intoimg3.".temp.jpg";
	   $this->turnOutImg($intoimg2,$intoimg3,$outimg2,$outimg3); 
	   
	   $image = imagecreatefrompng($bgpng);
	   $insert = imagecreatefrompng($outimg2);
	   $image = $this->image_overlap($image, $insert);
	   imagepng($image,$temByBgpng);
	   
	   $image = imagecreatefrompng($temByBgpng);
	   $insert = imagecreatefrompng($outimg3);
	   $image = $this->image_overlap($image, $insert);
	   imagepng($image,$temimgs);
	
	   $image = imagecreatefrompng($temimgs);
	   $insert = imagecreatefrompng($intoimg1);
	   imagejpeg($insert , $temjpgs);
	   $insert = imagecreatefromjpeg($temjpgs);
	
	   $image = $this->image_overlap($image, $insert);
	   if(!empty($resultimg)) {
		imagepng($image,$resultimg);
	   }else{
		imagepng($image);
	   }  
	   @unlink($temByBgpng);
...
function turnOutImg($intoimg1,$intoimg2,$outimg1,$outimg2) {
	   $im = $this->imageRotateAlpha($intoimg1 , -15);
	   imagepng($im , $outimg1); 
	   $im = $this->imageRotateAlpha($intoimg2 , 15);
	   imagepng($im ,$outimg2); 
	   imagedestroy($im);
}      
	
function imageRotateAlpha($img , $degrees) {
	   $source = imagecreatefrompng($img);
	   $im = $this->imageRotateOwn($source, $degrees, -1);
...
function imageCustomSizes($fromimg , $makeimg , $make_w , $make_h) { 	
	  $getsizearr = getimagesize($fromimg);
	  $W = $getsizearr[0];
	  $H = $getsizearr[1];
	  $maxsize = ($make_w>$make_h) ? $make_w : $make_h;
	  $this->setSrcImg($fromimg);
	  $this->setCutType(1);     
	  $this->setDstImg($makeimg);
	  $this->createImg($maxsize , $maxsize);
	 if($W > $make_w || $H > $make_h) {
	  $new_w = intval($maxsize/2 - $make_w/2);
	  $new_h = intval($maxsize/2 - $make_h/2);
	  $out_w = $make_w;
	  $out_h = $make_h;
	 } else {
	  $new_w = $make_w;
	  ...
	  $this->setSrcImg($makeimg);
	  $this->setCutType(2);
	  $this->setSrcCutPosition($new_w, $new_h);
	  $this->setRectangleCut($out_w, $out_h);
	  $this->setDstImg($makeimg);
	  $this->createImg($out_w, $out_h);  
} 
